...
class IotDeviceTable(models.Model):
	...
	def __unicode__(self):
		return u"{0}-{1}-{2}".format(self.device_addr, self.device_name, self.device_type)
	...
	def get_download_content(self):
		return u'\t'.join(map(unicode, [self.edit_time.strftime('%Y-%m-%d %H:%M:%S'),
		                                ...
		                                self.device_ov_password, self.device_web_password,
		                                self.device_software_version])) + u'\n'
...
class MissionDetailTable(models.Model):
	...
	def get_download_content(self):
		return u'\t'.join(map(unicode, [self.iot_device_id.device_addr, self.iot_device_id.device_name,
		                                ...
		                                self.can_discover, self.can_add, self.can_preview, self.can_calculate,
		                                self.can_delete,
		                                self.comment])) + u'\n'
