...
def generate_hash_id(str_in=str(time.time() * 1000)):
    """
    generate unique id based on time(ms) or given str
    :param str_in: source string
    :return: hash id
    """
    salt = str(random.random())
    m = hashlib.md5()
    m.update(str_in + salt)
    key = m.hexdigest()[:16]
    del m
            ...
            if reply.status_code == expected_status_code:
                ...
                if call_back_func:
                    try:
                        ...
                        self.logger.error("ERROR:call func {0} with data {1}, {2}".format(call_back_func, call_back_data, e))
            else:
