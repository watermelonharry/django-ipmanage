# -*- coding: UTF-8 -*
"""
__VERSION__ = '2017.09.18'
__author__ = 'harry'

description: module to call api module
change list:
0918: 新建文件
"""
...
class DispatcherThread(threading.Thread):
    def __init__(self, **kwargs):
        ...
        self.__ak = kwargs.get('ak', '')
        self.__terminal_name = kwargs.get('terminal_name', 'unknown')
...
class RegisterThread(threading.Thread):
    ...
    def set_status(self, status):
        ...
        self.info_dict['terminal_status'] = status
        self.logger.info("set terminal status to {0}".format(status))
        self.LOCK.release()
    ...
    def post(self, info_dict=None, post_url=None):
        ...
        try:
            ...
            if reply.status_code == 200:
                re_dict = reply.json()
                self.logger.info("get reply:{0}".format(re_dict))
                if re_dict.get("result", "error") == "error":
                    self.logger.error("register fail:{0}".format(re_dict.get("errors", None)))
                else:
                    return re_dict
            else:
                self.logger.error("status_code {0}".format(reply.status_code))
                self.logger.error("content:{0}".format(reply.json()))
...
class TestThread(threading.Thread):
    ...
    def run(self):
        for key, val in self.kdict.items():
            print("  [*] from {0}--{1}:{2}".format(self.__class__, key, val))
            time.sleep(4)


if __name__ == '__main__':
    k = RegisterThread(ak='8c9cd5f4383e65023472', terminal_name='regi_terminal_35',
                       api_root_url="http://192.168.0.108:8000")
