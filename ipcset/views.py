...
def show_settings_info(request):
    ...
    setting_list = VideoSettingTable.get_settings_by_username(user_name)
...
@login_required
def download_settting_table(request):
    user_name = request.user.username
    setting_list = (k.get_download_content() for k in VideoSettingTable.get_settings_by_username(user_name))
    response = StreamingHttpResponse(setting_list, content_type='APPLICATION/OCTET=STREAM')
    response['Content-Disposition'] = 'attachment; filename=setting_list.dat'
    return response
        ...
        if serializer.is_valid():
            ...
            for single_setting in setting_set:
                detail = MissionDetailTable(mission_id=mission_id,
                                            ...
                                            user_name=single_setting.user_name)
                detail.save()
